...
import { updateRosterStatus } from './roster.js';
...
export async function takeAdvancedOrder(custName, timeToReceive) {
    ...
    let existingCusts = (myRecord && myRecord.customerName) ? myRecord.customerName.split(', ').map(c=>c.trim()) : [];
    if (!existingCusts.includes(custName)) {
        await updateRosterStatus('Catering', custName);
    }
}
