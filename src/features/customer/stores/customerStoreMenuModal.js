// src/features/customer/stores/customerStoreMenuModal.js
...
import { 
    ...
    customizerQty, 
    isPreviewMode, 
    setActiveViewingStoreId, 
    setActiveViewingCategoryId, 
...
export function openCustomerStoreMenu(storeId) {
    setActiveViewingStoreId(storeId);
    setActiveViewingCategoryId(null);
...
export function renderStoreMenuItems(storeId) {
    ...
    if (displayCategories.length > 0) {
        ...
        if (!currentCatValid || !activeViewingCategoryId || activeViewingCategoryId === 'ALL') {
            setActiveViewingCategoryId(displayCategories[0].name);
        }
    } else {
        setActiveViewingCategoryId(null);
    }
...
export function selectCustomerMenuCategory(catName) {
    setActiveViewingCategoryId(catName);
...
export function openItemCustomizerModal(storeId, itemId) {
    ...
    setActiveCustomizingItem({ ...item, storeId });
    setCustomizerQty(1);
...
export function adjustCustomizerQty(delta) {
    setCustomizerQty(Math.max(1, customizerQty + delta));
