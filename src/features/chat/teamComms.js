...
export function initDraggableChat() {
    ...
    const onMove = (e) => {
        ...
        if (Math.abs(deltaY) > 10) {
            isDragging = true;
            let newTop = Math.max(60, Math.min(initialTop + deltaY, window.innerHeight - 80));
...
export function sendBubbleChatMessage() {
    ...
    if (isEveryoneTagged) showToast("📢 Tagged @everyone in Team Comms!");
}

export function triggerTeamChatImage() {
    document.getElementById('team-chat-image-input')?.click();
}
...
export function handleTeamChatImageFile(event) {
    ...
    showToast(isHdMode ? "📸 Processing HD photo..." : "⚡ Compressing photo...");
    compressAndResizeImage(file, isHdMode, (imageDataUrl) => {
        if (db) {
            db.ref('chat').push({
