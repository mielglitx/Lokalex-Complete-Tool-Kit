// src/features/chat/customerChat/custChatUI.js
import { appState } from '../../../store/state.js';
import { escapeHtml } from '../../../utils/helpers.js';
import { custChatState, MAPS_API_KEY } from './custChatState.js';

export function renderMessageStatusIndicator(msg) {
...
export function renderReactionsHtml(reactions, msgId, context = 'customer') {
...
export function showCustInChatToast(container, senderName, previewText) {
...
export function hideCustInChatToast() {
    const toast = document.getElementById('cust-inchat-newmsg-toast');
    if (toast) toast.classList.add('hidden');
}

export function showCustTopSpinner(container) {
...
export function hideCustTopSpinner() {
    const spinner = document.getElementById('cust-chat-history-spinner');
    if (spinner) spinner.remove();
}

export function renderCustomerMessages(container, isInitialLoad = false, oldScrollHeight = 0) {
    const msgs = Array.from(custChatState.loadedCustMsgsMap.values()).sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));

    if (msgs.length > 0) {
        custChatState.oldestCustMsgTimestamp = msgs[0].timestamp || null;
    }
    ...
    let topHeader = '';
    if (!custChatState.hasMoreCustMsgs) {
