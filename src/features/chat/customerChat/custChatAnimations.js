// src/features/chat/customerChat/custChatAnimations.js
import { custChatState, tapTrackerMap } from './custChatState.js';
...
export function handleCustMsgPointerDown(e, msgId, chatType, text, sender) {
    ...
    custChatState.startX = e.clientX ?? (e.touches ? e.touches[0].clientX : 0);
    custChatState.startY = e.clientY ?? (e.touches ? e.touches[0].clientY : 0);

    clearTimeout(custChatState.longPressTimer);

    custChatState.longPressTimer = setTimeout(() => {
        ...
        custChatState.longPressTimer = null;
    }, 450);
}

export function handleCustMsgPointerMove(e) {
    if (!custChatState.longPressTimer) return;
    const currentX = e.clientX ?? (e.touches ? e.touches[0].clientX : 0);
    const currentY = e.clientY ?? (e.touches ? e.touches[0].clientY : 0);

    if (Math.abs(currentX - custChatState.startX) > 10 || Math.abs(currentY - custChatState.startY) > 10) {
        clearTimeout(custChatState.longPressTimer);
        custChatState.longPressTimer = null;
    }
}
...
export function handleCustMsgPointerUp(e, msgId) {
    if (custChatState.longPressTimer) {
        clearTimeout(custChatState.longPressTimer);
        custChatState.longPressTimer = null;
