...
/**
 * ============================================================================
 * RIDER ACCOUNTS & ROLE PERMISSION MANAGEMENT
 * ============================================================================
 * 
 * Controls account provisioning, role transitions (Admin, TL, Rider), Team Lead
 * granular privilege matrices, and account deletion across all state stores.
 * 
 * Update Note:
 * - Synchronizes userType mutations across riders/, roster/, and settings/userTypes.
 * - Forces memory and local cache persistence to stop riders from reverting to Admin.
 * ============================================================================
 */
        ...
        container.innerHTML = ridersList.map(r => {
            ...
            <div class="bg-black/30 border border-gray-800 p-3 rounded-2xl flex items-center justify-between gap-2 shadow text-xs">
                ...
                <div class="flex items-center gap-1.5 shrink-0">
                    <select onchange="window.quickChangeRiderUserType && window.quickChangeRiderUserType('${r.id}', this.value)" class="bg-gray-900 text-[10px] font-bold border border-gray-700 rounded-lg px-2 py-1 text-gray-200 cursor-pointer">
...
/**
 * Authoritative role transition function.
 * Updates riders/, roster/, and settings/userTypes synchronously while refreshing memory caches.
 */
export async function quickChangeRiderUserType(riderId, newUserType) {
    ...
    try {
        ...
        // Synchronize in-memory roster entries
        if (existingMem) {
