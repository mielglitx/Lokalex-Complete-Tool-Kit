...
import { 
    ...
    openAdminQueueSettingsModal 
} from '../rosterQueueSettings.js';
...
export function initQueueCooldownTicker() {
    if (queueCooldownTickerInterval) {
        ...
        queueCooldownTickerInterval = null;
    }

    queueCooldownTickerInterval = setInterval(() => {
        const badges = document.querySelectorAll('.queue-cooldown-timer');
        ...
        badges.forEach(badge => {
            const targetTime = parseInt(badge.getAttribute('data-cooldown-until'), 10);
            if (!targetTime) return;

            const remainingMs = targetTime - currTime;
            if (remainingMs <= 0) {
                needsResort = true;
            } else {
                ...
                const digitsEl = badge.querySelector('.timer-digits');
                const formatted = `${mins}:${String(secs).padStart(2, '0')}`;

                if (digitsEl) {
                    if (digitsEl.innerText !== formatted) {
                        digitsEl.innerText = formatted;
                    }
                } else {
                    badge.innerHTML = `⏳ <span class="timer-digits">${formatted}</span>`;
                }
            }
        });
...
export function updateRosterUI() {
    ...
    const queueSettingsBtn = document.getElementById('admin-queue-settings-btn');
    if (queueSettingsBtn) {
        if (showControls && isAdmin()) queueSettingsBtn.classList.remove('hidden');
    ...
    const queueSettings = getQueueLineupSettings();
    const isLowestGrossMode = (queueSettings.mode || 'lowest_gross') === 'lowest_gross';
    const cooldownMins = parseInt(queueSettings.cooldownMinutes, 10) || 0;
    ...
    availableRiders.forEach((m) => {
        ...
        const rawTs = m.availableTimestamp || parseQueueTime(m.queueTime) || 0;
        let cooldownBadge = "";

        if (isLowestGrossMode && cooldownMs > 0 && rawTs > 0) {
            const cooldownUntil = rawTs + cooldownMs;
            const elapsed = now - rawTs;
            if (elapsed < cooldownMs) {
                const remSecs = Math.max(0, Math.ceil((cooldownUntil - now) / 1000));
                const mins = Math.floor(remSecs / 60);
                const secs = remSecs % 60;
                const timeTag = `${mins}:${String(secs).padStart(2, '0')}`;
                cooldownBadge = ` <span class="queue-cooldown-timer inline-flex items-center gap-1 text-[9px] font-mono font-bold text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 px-1.5 py-0.5 rounded cursor-default select-none" data-cooldown-until="${cooldownUntil}" title="Queue Cooldown Active: Placed at the end of the line for ${cooldownMins}m">⏳ <span class="timer-digits">${timeTag}</span></span>`;
            }
        }
