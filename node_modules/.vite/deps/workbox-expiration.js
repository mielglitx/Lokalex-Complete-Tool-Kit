...
var CacheExpiration = class {
	...
	*/
	constructor(cacheName, config = {}) {
		...
		if (!(config.maxEntries || config.maxAgeSeconds)) throw new WorkboxError("max-entries-or-age-required", {
			moduleName: "workbox-expiration",
			className: "CacheExpiration",
			funcName: "constructor"
		});
	...
	*/
	async isURLExpired(url) {
		if (!this._maxAgeSeconds) throw new WorkboxError(`expired-test-without-max-age`, {
		...
		*/
		this.cachedResponseWillBeUsed = async ({ event, request, cacheName, cachedResponse }) => {
			...
			if (event) try {
				...
				if ("request" in event) logger.warn(`Unable to ensure service worker stays alive when updating cache entry for '${getFriendlyURL(event.request.url)}'.`);
			}
