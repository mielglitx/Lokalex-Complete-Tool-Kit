import { i as WorkboxError, n as logger, r as finalAssertExports, t as getFriendlyURL } from "./getFriendlyURL-DM31Tn_2.js";
import { n as cacheNames, t as quotaErrorCallbacks } from "./quotaErrorCallbacks-DQ5qTn8h.js";
...
var StrategyHandler = class {
	...
	*/
	async cachePut(key, response) {
		...
		{
			if (effectiveRequest.method && effectiveRequest.method !== "GET") throw new WorkboxError("attempt-to-cache-non-get-request", {
				url: getFriendlyURL(effectiveRequest.url),
				method: effectiveRequest.method
			});
			const vary = response.headers.get("Vary");
			if (vary) logger.debug(`The response for ${getFriendlyURL(effectiveRequest.url)} has a 'Vary: ${vary}' header. Consider setting the {ignoreVary: true} option on your strategy to ensure cache matching and deletion works as expected.`);
		}
		if (!response) {
		...
		if (!pluginsUsed) {
			...
			if (responseToCache) {
				if (responseToCache.status !== 200) {
					if (responseToCache.status === 0) logger.warn(`The response for '${this.request.url}' is an opaque response. The caching strategy that you're using will not cache opaque responses by default.`);
	...
	*/
	...
	async _getResponse(handler, request, event) {
		...
		try {
			...
			else logger.log(`While responding to '${getFriendlyURL(request.url)}', an ${error instanceof Error ? error.toString() : ""} error occurred. Using a fallback response provided by a handlerDidError plugin.`);
		}
...
*/
//#endregion
export { StrategyHandler as n, timeout as r, Strategy as t };
